...
let volSlider = document.getElementsByClassName("volSlider")[0];
let skip = document.getElementsByClassName("skip")[0];
...
// retrieve the input value from the volume slider and send it to changeVolume function
volSlider.addEventListener("input", (e) => {
  changeVolume(e.target.value)
});

//change volume when called
...
// create a list of tracks for the player to seek through
let urlArray = [
...
function changeTrack(){
  ...
  play.setAttribute("data-icon", "P"); /* set the pause icon back to play */
  if(currentTrack > 3){
    currentTrack = 0;
  }

  media.src = urlArray[currentTrack];
